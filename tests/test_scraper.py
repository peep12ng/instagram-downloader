...
async def test_scrape_profile_page_not_found(mock_get_authenticated_page):
    ...
    mock_not_found_locator.is_visible.return_value = True
    mock_page.locator.return_value = mock_not_found_locator

    # get_authenticated_page가 가짜 Page 객체를 반환하도록 설정
...
async def test_scrape_profile_page_is_private(mock_get_authenticated_page):
    ...
    mock_post_locator.count.return_value = 0

    # page.locator가 호출 순서에 따라 다른 mock 객체를 반환하도록 설정
    mock_page.locator.side_effect = [mock_not_found_locator, mock_post_locator]
